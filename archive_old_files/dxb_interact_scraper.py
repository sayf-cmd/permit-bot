...
def find_best_location(building_name):
    ...
    for item in items:
        ...
        if "downtown" in target and "grande" in query:
            score += 0.8
...
def format_result(data):
    ...
    active_rent = None
    for start_date, rent_amount, rent_type, end_date in rents:
    ...
    status = "🔴 Rented" if active_rent else "🟢 Available"

    lines = [
        f"🏢 Building: {data.get('building', '-')}",
        f"📍 Area: {data.get('area', '-')}",
        f"🏠 Unit Number: {data.get('unit_number', '-')}",
        f"🆔 Trakheesi / PROP ID: {data.get('prop_id', '-')}",
        "",
        f"🛏 Rooms: {data.get('bedrooms', '-')}",
    ...
    if active_rent:
        lines.extend([
            f"📅 Rented Until: {active_rent['end']}",
            f"💵 Current Rent: AED {active_rent['amount']} — {active_rent['type']}",
        ])
    ...
    if data.get("sales"):
        for date, price, seller in data["sales"]:
            lines.append(f"• {date} — AED {price} — {seller}")
    else:
        lines.append("• No sale transactions found")
    ...
    if rents:
        for start_date, rent_amount, rent_type, end_date in rents:
            lines.append(
                f"• {start_date} → {end_date} — AED {rent_amount} — {rent_type}"
            )
    else:
        lines.append("• No rent transactions found")
...
async def search_dxb_unit(building_name: str, unit_number: str) -> str:
    ...
    async with async_playwright() as p:
        ...
        print("BODY AFTER SEARCH:")
        print(body_after_search[:3000], flush=True)

        print("BODY SALE:")
        print(body_sale[:3000], flush=True)

        print("BODY RENT:")
        print(body_rent[:3000], flush=True)
        ...
        () => {
            const el = document.querySelector("#P142_PROP_ID");
            return el ? el.value : "";
        }
        """)
        ...
        area = "-"
        if "," in location_name:
            area = location_name.split(",")[-1].strip()

        data_base["building"] = location_name
        data_base["area"] = area
        data_base["unit_number"] = unit_number
        data_base["prop_id"] = prop_id or "-"
