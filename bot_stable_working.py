...
try:
    if SHEET_CSV_URL and SHEET_CSV_URL != "test":
        ...
        print("Permit CSV disabled locally — skipped")
        df = None
        permit_col = ""
        building_col = ""
        unit_col = ""
...
def add_search_history(*args, **kwargs):
    return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Local-safe mode: do not require Google Service Account for permit/PF search.
        row_number = None
        record = {
            "status": "admin",
            "requests_used": 0,
        ...
        reply = (
            ...
            f"🧑 Name: {str(row.get('Latest_Owner', '-')).title()}\n"
